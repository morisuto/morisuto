# morisuto

双子パパエンジニア / Full Cycle Engineer

## Career

- **identify株式会社**（2024/08〜）
  - Full Cycle Engineer — 開発部 / DeLMO など
- **インフォメティス株式会社**（2022/07〜2024/07）
  - Application Engineer — IoTソリューション事業部 / enenowa など
- **レバレジーズグループ**（2016/04〜2022/06）
  - Web Engineer — HRテック事業部（2020/11〜2022/06）
  - Sales — レバレジーズメディカルケア リハ事業部（2020/09〜2020/10）
    - ※海外事業部在籍時からエンジニア転向の希望を出していたが、Singapore 支社撤退でリソースが余っていたため新規事業（リハ事業）の立ち上げに協力。異動許可が出たタイミングでエンジニア職へ移動
  - Sales — 海外事業部 / Leverages Career Singapore（2019/07〜2020/08）
  - Sales — レバテック ITS事業部（2016/04〜2019/06）

### 直近 — identify株式会社（2024/08〜）

広告用動画素材プラットフォーム「DeLMO for Ad」の開発。企画から設計・実装・運用まで一気通貫で担当（フロント・バック・インフラ・データ分析・AI）。

#### 仕事への向き合い方

- **VMV（Vision / Mission / Values）から逆算した技術選定**：エンジニアリングを「事業戦略を実現する戦術の一つ」と捉え、売上・競合優位に繋がる実装を見極める
- **越境する広い担当領域**：フロントエンド / バックエンド / インフラ を軸に、機械学習 / データ分析 / 採用 にも半歩越境中。事業課題に応じて領域をまたいで手を動かす
- **Full Cycle × AI 活用で完遂する**：企画〜設計〜実装〜リリース〜改善まで Full Cycle で一気通貫。Claude Code / Codex / Devin / CodeRabbit などを適材適所で駆使

#### 主な技術スタック

- フロントエンド（React, TypeScript, Next.js, Apollo Client, Tailwind CSS, Turborepo）
- バックエンド（Go, GraphQL, gqlgen, SQLBoiler）
- インフラ（Google Cloud, Terraform, Packer）
- 動画処理（FFmpeg, NVENC, H.264/H.265, CUDA）
- 検索 / 決済 / データ分析（Elasticsearch, Stripe, Redash, BigQuery）
- AI ツール（Vertex AI, Gemini, Claude Code, CodeRabbit）

## Study

- **University of the People**
  - Computer Science, B.Sc.（2026/06 卒業見込み）
  - Computer Science, Associate's degree（2021/06〜2025/11）
- **Waseda University**
  - Physics and Applied Physics, M.Eng.（2016/03 中退）
  - Applied Physics, B.Eng.（2015/03 卒業）

※University of the People は社会人オンライン大学。業務時間外で学位を取得中

## Certifications

- TOEIC 885（2021/04）
- 基本情報技術者試験（2021/12）
- TOEFL iBT 90/120（2026/04）

## Interests

- 家族時間 / 卓球指導 / アウトドア
- パターン認識 / 動画検索 / レコメンデーション
- ビジネス戦略（3C/4P）から逆算するエンジニアリング

## Blog

- note: [@ren_morisuto](https://note.com/ren_morisuto) — 双子育児とスタートアップ開発の話など

## Tech Stack

**Backend**
![Go](https://img.shields.io/badge/-Go-00ADD8?logo=go&logoColor=white)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?logo=typescript&logoColor=white)
![Node.js](https://img.shields.io/badge/-Node.js-339933?logo=nodedotjs&logoColor=white)
![Python](https://img.shields.io/badge/-Python-3776AB?logo=python&logoColor=white)
![GraphQL](https://img.shields.io/badge/-GraphQL-E10098?logo=graphql&logoColor=white)

**Frontend**
![React](https://img.shields.io/badge/-React-61DAFB?logo=react&logoColor=black)
![Next.js](https://img.shields.io/badge/-Next.js-000000?logo=nextdotjs&logoColor=white)
![Tailwind](https://img.shields.io/badge/-Tailwind-06B6D4?logo=tailwindcss&logoColor=white)

**Infra / Cloud**
![Google Cloud](https://img.shields.io/badge/-Google_Cloud-4285F4?logo=googlecloud&logoColor=white)
![Terraform](https://img.shields.io/badge/-Terraform-7B42BC?logo=terraform&logoColor=white)
![Docker](https://img.shields.io/badge/-Docker-2496ED?logo=docker&logoColor=white)

**Media / Data**
![FFmpeg](https://img.shields.io/badge/-FFmpeg-007808?logo=ffmpeg&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-4169E1?logo=postgresql&logoColor=white)
![MySQL](https://img.shields.io/badge/-MySQL-4479A1?logo=mysql&logoColor=white)
